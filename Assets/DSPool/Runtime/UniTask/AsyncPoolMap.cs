...
public class AsyncPoolMap<TPoolKey, TPool, TPoolElement>
    where TPoolElement : class
    where TPool : AsyncObjectPool<TPoolElement>
{
    ...
    public async UniTask<TPoolElement> RentAsync(TPoolKey poolKey)
    {
        var pool = this.GetPoolByKey(poolKey);
        var rentedElement = await pool.RentAsync();
